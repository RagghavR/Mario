var mario,ground,ground2,clouds,coins,start,bush,hill,mushroom,bricks,coinbrick,hole,goomba,gameover
var marioI,marioI1,marioI2,marioI3,groundI,cloudsI1,cloudsI2,cloudsI3,coinsI,startI,jumpI,bush1,bush2,bush3,squash,hillI,mushroomI,mushroomI2,bricksI,coinbrickI,coinsbrickI,holeI,goombaI,goombaI2,gameoverI
...
function draw() {
    ...
    if(gamestate === "play" && keyDown("space") && mario.collide(floorg)){
        mario.velocityY = -20
        jump.play()
    }
    if(ground.x <= 500){
    ...
    if(gamestate === "restart" && keyDown("space")){
        gamestate = "play"
        mario.scale = 0.25
        mario.changeAnimation("run",marioI)
        mario.setCollider("rectangle",0,-30,320,350)
    ...
    if(mario.isTouching(goombag.get(i)) && mario.velocityY > 0 && gamestate === "play"){
    ...
    if(gamestate === "play" && mario.isTouching(obsg) && lives >= 1 && state===1 && mario.velocityY < 0){
        gamestate = "end"
        death.play()
    }
    if(gamestate === "play" && mario.isTouching(obsg)&& state===2 && mario.velocityY < 0){
        mario.changeAnimation("run",marioI)
        mario.y = height*0.55
        state = 1
        powerdown.play()
    }
    if(gamestate === "play" && mario.isTouching(obsg) && lives <= 0 && state===1){
        gamestate = "die"
        song.stop()
        gameovers.play()
    }
    ...
    if (gamestate === "death" && mario.y >= height*1.05){
        gamestate = "restart"
    }
    if(keyDown("up_arrow")){
        state = 2
...
function holes(){
    hole = createSprite(width*1.5,height*0.955,10,10)
    if(gamestate==="play"){
        hole.velocityX = ground.velocityX
    }
    hole.addImage(holeI)
    hole.lifetime = 1200
    hole.depth = bush.depth - 1
    hole.scale = 0.25
    obsg.add(hole)
    holeg.add(hole)
    hole.depth = ground.depth + 1
    hole.debug = true
